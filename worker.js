      ...
      for (const conv of conversations) {
        ...
        for (let page = 0; page < MAX_PAGES; page++) {
          ...
          for (const msg of messages) {
            ...
            // Attachments first — accept any recording extension, OR ANY attachment
            // on a call-type message (GHL signed URLs may lack an extension).
...
async function writeToSheet(env, callData, transcript, aiExtracted) {
  ...
  // Stamp the recording URL fingerprint into column U (separate write).
  if (callData.recordingUrl) {
