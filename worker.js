...
async function processCall(payload, env, attempts = 1) {
  ...
  if (callData.allRecordingUrls.length > 0) {
    ...
    candidates = callData.allRecordingUrls.filter(u => !processed.has(normalizeRecordingUrl(u)));
...
function extractContactInfo(payload) {
  ...
  return {
    contactId, sellerName, phone,
    callDate: new Date(callDate).toISOString(),
    propertyAddress: address1, cityState: cityState.trim(),
...
async function fetchGHLCallData(contactId, env) {
  ...
  const headers = { "Authorization": "Bearer " + env.GHL_API_KEY, "Version": "2021-07-28" };
...
async function getProcessedRecordingUrls(env) {
  try {
    ...
    if (!resp.ok) { console.error("Failed to read processed URLs:", resp.status); return new Set(); }
    const data = await resp.json();
    const urls = (data.values || []).map(row => (row && row[0]) || "").filter(Boolean).map(normalizeRecordingUrl);
...
async function writeToSheet(env, callData, transcript, aiExtracted) {
  ...
  const firstDataRow = parseInt(env.FIRST_DATA_ROW || "5");
  const accessToken = await getGoogleAccessToken(env);
  if (!accessToken) { console.error("Failed to get Google access token"); return; }
  const sheetId = env.GOOGLE_SHEET_ID;
  ...
  if (lastMatchRow !== -1 && lastMatchRow + 1 < appendRow) {
    ...
    if (numericSheetId !== null) {
      ...
      if (!inserted) targetRow = appendRow;
    } else {
      targetRow = appendRow;
    }
  } else {
  ...
  const rowValues = [
    dateStr, callData.sellerName, callData.phone, address, cityState, propType,
    aiExtracted.beds || "", aiExtracted.baths || "", aiExtracted.sqft || "", price,
    aiExtracted.amount_owed || "", aiExtracted.repairs_needed || "", aiExtracted.occupancy_access || "",
    aiExtracted.motivation || "", aiExtracted.key_dates || "", aiExtracted.call_summary || "",
    aiExtracted.acquisitions_coaching || "", aiExtracted.next_steps || "", aiExtracted.callback_date || "",
  ];
  ...
  try {
    const resp = await fetch(`${baseUrl}:batchUpdate`, {
      ...
      body: JSON.stringify({
        requests: [{
          insertDimension: {
            range: { sheetId: numericSheetId, dimension: "ROWS", startIndex: rowNumber - 1, endIndex: rowNumber },
